...
inline bool BacklinksCompression ::Exist(const std::vector<std::vector<int>> &adj, int from, int to)
{
  return std::binary_search(adj[from].begin(), adj[from].end(), to);
}

inline void BacklinksCompression ::ProceedCopying(const std::vector<std::vector<int>> &adj, int val,
                                                  int now, int *cur, std::vector<int> *residual)
{
